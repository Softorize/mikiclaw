...
async function resolveGacBinary(): Promise<string | null> {
  const home = process.env.HOME || '';
  const candidates = [process.env.GAC_BIN, home ? `${home}/go/bin/gac` : undefined, 'gac'].filter(
...
async function runGacJson<T>(binary: string, args: string[]): Promise<T | null> {
  ...
  try {
    ...
    logger.error('Failed to parse gac JSON output', {
      args,
      raw: raw.slice(0, 500),
      error: String(error),
    });
...
export async function gacListProperties(accountId?: string): Promise<string> {
  ...
  try {
    ...
    const accountNameById = new Map(
      scopedAccounts.map(account => [account.ID, account.DisplayName])
    );
    ...
    for (const account of scopedAccounts) {
      try {
        const properties =
          (await runGacJson<GacProperty[]>(binary, [
            'properties',
            'list',
            '--account',
            account.ID,
    ...
    const lines: string[] = [];
    lines.push(
      `📈 GA properties found: ${allProperties.length} across ${scopedAccounts.length} account(s).`
    );
