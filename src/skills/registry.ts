...
class SkillsRegistry {
  ...
  async list(): Promise<void> {
    ...
    const skills = readdirSync(skillsDir).filter(f => !f.startsWith('.') && f !== 'node_modules');
  ...
  async install(skillName: string): Promise<void> {
    ...
    try {
      ...
      writeFileSync(join(skillPath, 'claw.json'), JSON.stringify(manifest, null, 2));
      ...
      spinner.fail(
        `Failed to install: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async search(query: string): Promise<void> {
    ...
    try {
      const response = await fetch(
        `https://clawhub.ai/api/skills/search?q=${encodeURIComponent(query)}`
      );
