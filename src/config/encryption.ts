...
export function encryptConfig(config: Record<string, unknown>): Record<string, unknown> {
  ...
  if (
    ...
    !telegram.botToken.startsWith('encrypted:')
  ) {
  ...
  if (
    ...
    !anthropic.apiKey.startsWith('encrypted:')
  ) {
  ...
  if (
    slack?.appToken &&
    typeof slack.appToken === 'string' &&
    !slack.appToken.startsWith('encrypted:')
  ) {
    slack.appToken = `encrypted:${encrypt(slack.appToken)}`;
  }
  if (
    slack?.botToken &&
    typeof slack.botToken === 'string' &&
    !slack.botToken.startsWith('encrypted:')
  ) {
    slack.botToken = `encrypted:${encrypt(slack.botToken)}`;
  }
  if (
    slack?.signingSecret &&
    typeof slack.signingSecret === 'string' &&
    !slack.signingSecret.startsWith('encrypted:')
  ) {
  ...
  if (
    ...
    !discord.botToken.startsWith('encrypted:')
  ) {
