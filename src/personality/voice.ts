...
class AIVoiceTracker {
  ...
  analyzeResponse(userId: string, response: string): void {
    ...
    voice.responseStyle.avgLength = Math.round((oldAvg * (total - 1) + response.length) / total);
    ...
    // Check enthusiasm
    const enthusiasmCount = VOICE_INDICATORS.enthusiastic.filter(e =>
      lowerResponse.includes(e)
    ).length;
