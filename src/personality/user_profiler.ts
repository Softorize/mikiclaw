...
class UserProfiler {
  ...
  analyzeMessage(message: string): void {
    ...
    for (const [topic, keywords] of Object.entries(topicKeywords)) {
      if (keywords.some(k => lowerMessage.includes(k))) {
        ...
        profile.preferences.interests = this.addInterest(profile.preferences.interests, topic);
      }
    }
  ...
  trackHumorReaction(
    ...
    for (const t of types) {
      ...
      if (total >= 3) {
        // Need at least 3 interactions to establish preference
